...
const Pagination = (props) => {
    ...
    return (
        <div className={`custom-pagination flex w-[100%] justify-between ${props?.className}`}>
            <ul className="showItems">
                ...
                </li>{label || ''} {label ? "per" : 'Per'}  page
            </ul>
            <ul className="pagination flex">
                <li className="pagination-item">
                    <button onClick={() => onClickFirstPage()} className={isInFirstPage() ? 'disabled' : ''} disabled={isInFirstPage()}><SkipPreviousFilled /></button>
                </li>
                <li className="pagination-item">
                    <button onClick={() => onClickPreviousPage()} className={isInFirstPage() ? 'disabled' : ''} disabled={isInFirstPage()}><BackIcon /></button>
                </li>
                {pages}
                <li className="pagination-item">
                    <button onClick={() => onClickNextPage()} className={isInLastPage() ? 'disabled' : ''} disabled={isInLastPage()}><NextIcon /></button>
                </li>
                <li className="pagination-item">
                    <button onClick={() => onClickLastPage()} className={isInLastPage() ? 'disabled' : ''} disabled={isInLastPage()}> <SkipNextFilled /> </button>
                </li>
            </ul>
        </div>
